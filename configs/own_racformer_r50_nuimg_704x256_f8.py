...
dataset_root = "/mnt/diskNvme0/wangpengData/OwnData/"
...
# ========== 时序配置（可调整）==========
num_frames = 8          # 总帧数（1个当前帧 + 7个历史帧）
sweeps_num = 7          # 历史帧数量
sweep_interval = 2      # 帧间隔（15Hz数据，间隔2帧 ≈ 0.133秒）
...
ida_aug_conf = {
    ...
    'rot_lim': (0.0, 0.0),
    'H': 720,   # 修改为自定义数据的图像高度
    'W': 1280,  # 修改为自定义数据的图像宽度
    'rand_flip': False,  # 测试模式不翻转
}
...
# ========== 简化的测试Pipeline（只使用当前帧radar数据）==========
test_pipeline = [
    dict(type='LoadMultiViewImageFromFiles', to_float32=False, color_type='color'),
    dict(type='LoadMultiViewImageFromMultiSweeps', sweeps_num=sweeps_num, test_mode=True),
    dict(type='RandomTransformImage', ida_aug_conf=ida_aug_conf, training=False),
    # 简化：只加载当前帧的radar数据，不使用历史帧
    dict(type='Loadnuradarpoints', coord_type='RADAR', num_sweeps=1, file_client_args=file_client_args),
    # 移除：LoadradarpointsFromMultiSweeps（需要nuScenes特定功能）
    dict(
    ...
    dict(
        ...
        img_scale=(1280, 720),  # 修改为自定义数据的图像分辨率
        pts_scale_ratio=1,
        flip=False,
        transforms=[
...
data = dict(
    workers_per_gpu=4,
    val=dict(  # 添加val配置（用于val.py）
        type=dataset_type,
        data_root=dataset_root,
        ann_file=dataset_root + 'own_infos.pkl',
    ...
    test=dict(
        type=dataset_type,
        data_root=dataset_root,
        ann_file=dataset_root + 'own_infos.pkl',  # 使用自定义数据的pkl文件
        pipeline=test_pipeline,
        classes=class_names,
        modality=input_modality,
        test_mode=True,
        box_type_3d='LiDAR',
        sweeps_num=sweeps_num,        # 传递给数据集
        sweep_interval=sweep_interval  # 传递给数据集
    )
)
...
# ========== 加载预训练权重 ==========
# 注意：这里应该使用训练好的RaCFormer模型权重，而不是预训练的backbone
load_from = 'checkpoints/racformer_r50_f8.pth'
revise_keys = []  # 不需要修改key，直接加载完整模型
