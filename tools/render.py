...
import sys
# sys.path.append("/chuxiaomeng/RaCFormer")
sys.path.append("/home/wangpeng/CODE/RaCFormer-main")
...
def visualize_sample_radar(nusc: NuScenes,
                     sample_token: str,
                     gt_boxes: EvalBoxes,
                     pred_boxes: EvalBoxes,
                     nsweeps: int = 1,
    ...
    # Retrieve sensor & pose records.
    sample_rec = nusc.get('sample', sample_token)
    ref_sd_record = nusc.get('sample_data', sample_rec['data']['LIDAR_TOP'])
    ...
    for token in rad_types: 
        ...
        sd_record = nusc.get('sample_data', sample_rec['data'][token])
        chan = sd_record['channel']

        pc, times = RadarPointCloud_v2.from_file_multisweep(nusc,
                        sample_rec, sample_rec, chan, 'LIDAR_TOP', nsweeps=nsweeps)
    ...
    # plt.title(sample_token)
    if savepath is not None:
        plt.savefig(savepath, bbox_inches="tight", pad_inches=0.5)
        plt.close()
    else:
        plt.show()
