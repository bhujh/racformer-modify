...
import sys
sys.path.append("/home/wangpeng/Code/RaCFormer")
from loaders.nuscenes_dataset import RadarPointCloud_v2
...
def lidar_render(sample_token, data, out_path: str = None):
    bbox_gt_list = []
    bbox_pred_list = []
...
def render_sample_data(
    ...
    all_radar_times = np.zeros((1, 0))

    for token in rad_types:
        sd_record = nusc.get('sample_data', sample['data'][token])
        chan = sd_record['channel']
        pc, times = RadarPointCloud_v2.from_file_multisweep(nusc, sample, sample, chan, 'LIDAR_TOP', nsweeps=6)
        all_radar_points = np.concatenate([all_radar_points, pc.points], axis=1)
        all_radar_times = np.concatenate([all_radar_times, times], axis=1)
    ...
    # 2. 定义一个内部函数来处理绘图和保存逻辑，避免代码重复
    def _plot_cameras(cam_list, view_title_suffix):
        # 为每组相机创建一个新的 2x3 画布
        ...
        for ind, cam in enumerate(cam_list):
            ...
            if sensor_modality == "camera":
                ...
                # 在 2x3 的网格中绘图，ind 从 0 到 2
                # 第一行 (ax[0, :]) 用于显示预测结果 (PRED)
                ax[0, ind].imshow(data)
                # 第二行 (ax[1, :]) 用于显示真值 (GT)
                ax[1, ind].imshow(data)
                ...
                if with_anns:
                    for box in boxes_pred:
                        c = np.array(get_color(box.name)) / 255.0
                        box.render(ax[0, ind], view=camera_intrinsic, normalize=True, colors=(c, c, c))
                    for box in boxes_gt:
                        c = np.array(get_color(box.name)) / 255.0
                        box.render(ax[1, ind], view=camera_intrinsic, normalize=True, colors=(c, c, c))


                # 投影聚合后的雷达点到相机图像
                if all_radar_points.shape[1] > 0:
                    cam_sd_record = nusc.get('sample_data', sample_data_token)
                    cam_cs_record = nusc.get('calibrated_sensor', cam_sd_record['calibrated_sensor_token'])
                    cam_pose_record = nusc.get('ego_pose', cam_sd_record['ego_pose_token'])
                    ...
                    lidar_sensor_to_ego = transform_matrix(lidar_cs_record['translation'], Quaternion(lidar_cs_record['rotation']), inverse=False)
                    lidar_ego_to_global = transform_matrix(lidar_pose_record['translation'], Quaternion(lidar_pose_record['rotation']), inverse=False)
                    global_to_cam_ego = transform_matrix(cam_pose_record['translation'], Quaternion(cam_pose_record['rotation']), inverse=True)
                    cam_ego_to_sensor = transform_matrix(cam_cs_record['translation'], Quaternion(cam_cs_record['rotation']), inverse=True)

                    trans_matrix = reduce(np.dot, [cam_ego_to_sensor, global_to_cam_ego, lidar_ego_to_global, lidar_sensor_to_ego])

                    points_lidar = all_radar_points[:3, :].copy()
                    points_cam = np.dot(trans_matrix[:3, :3], points_lidar) + trans_matrix[:3, 3:4]

                    depth = points_cam[2, :]

                    cam_intrinsic_mat = np.array(cam_cs_record['camera_intrinsic'])
                    points_img = view_points(points_cam, cam_intrinsic_mat, normalize=True)

                    valid_mask = (points_img[0, :] > 0) & (points_img[0, :] < data.size[0]) & \
                                 (points_img[1, :] > 0) & (points_img[1, :] < data.size[1]) & \
                                 (depth > 0)

                    if np.any(valid_mask):
                        points_valid = points_img[:2, valid_mask]
                        depth_valid = depth[valid_mask]
                        norm = plt.Normalize(vmin=depth_valid.min(), vmax=depth_valid.max())
                        colors = plt.cm.Reds(norm(depth_valid))
                        ax[0, ind].scatter(points_valid[0, :], points_valid[1, :], c=colors, s=2, alpha=0.5)
                        ax[1, ind].scatter(points_valid[0, :], points_valid[1, :], c=colors, s=2, alpha=0.5)
                

                # Limit visible range.
                ax[0, ind].set_xlim(0, data.size[0])
                ax[0, ind].set_ylim(data.size[1], 0)
                ax[1, ind].set_xlim(0, data.size[0])
                ax[1, ind].set_ylim(data.size[1], 0)

            # 设置子图的标题和格式
            ax[0, ind].axis("off")
            ax[0, ind].set_title("PRED: {} {}".format(sd_record["channel"], view_title_suffix))
            ax[0, ind].set_aspect("equal")

            ax[1, ind].axis("off")
            ax[1, ind].set_title("GT: {} {}".format(sd_record["channel"], view_title_suffix))
            ax[1, ind].set_aspect("equal")

        return fig

    # 3. 处理并保存前视图图像
    if out_path is not None:
        fig_front = _plot_cameras(cams_front, "(Front Views)")
        plt.savefig(out_path + "_camera_front.png", bbox_inches="tight", pad_inches=0.2, dpi=200)
        plt.close(fig_front)  # 关闭前视图的画布

    # 4. 处理并保存后视图图像
    if out_path is not None:
        fig_back = _plot_cameras(cams_back, "(Back Views)")
        plt.savefig(out_path + "_camera_back.png", bbox_inches="tight", pad_inches=0.2, dpi=200)
        plt.close(fig_back)  # 关闭后视图的画布
...
if __name__ == '__main__':
    ...
    results = mmcv.load("submission_mini/pts_bbox/results_nusc.json")
    sample_token_list = list(results['results'].keys())
    # print(len(sample_token_list))
    # print("+++++++++++++++++++++++++++++")
    # print(results["results"].keys())
    # print("+++++++++++++++++++++++++++++")
    # print(results.keys())
    
    # print(results['results'][sample_token_list[0]])
    # print(results["meta"])
    # os.abort()
    for id in range(0, 300):  # 6019
        render_sample_data(sample_token_list[id], pred_data=results, out_path="./pred_img_outputs_mini/" + str(sample_token_list[id]), verbose=False)
