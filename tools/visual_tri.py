...
def render_sample_data(
    ...
    if out_path is not None:
        ...
        plt.savefig(out_path + "_camera_front.png", bbox_inches="tight", pad_inches=0, dpi=200)
        plt.close(fig_front)  # 关闭前视图的画布

    # 4. 处理并保存后视图图像
    if out_path is not None:
        fig_back = _plot_cameras(cams_back, "(Back Views)")
        plt.savefig(out_path + "_camera_back.png", bbox_inches="tight", pad_inches=0, dpi=200)
...
if __name__ == '__main__':
    nusc = NuScenes(version="v1.0-trainval", dataroot="/mnt/diskNvme1/dataset/nuscenes", verbose=True)
    # results = mmcv.load('submission/pts_bbox/r50_f8_results_nusc.json')
    results = mmcv.load("submission/pts_bbox/results_nusc.json")
    ...
    # print(results["meta"])
    # os.abort()
    for id in range(0, 50):  # 6019
        render_sample_data(sample_token_list[id], pred_data=results, out_path="./pred_img_outputs/" + str(sample_token_list[id]), verbose=False)
